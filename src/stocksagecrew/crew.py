...
class Stocksagecrew():
    ...
    @agent
    def stock_explainer(self) -> Agent:
        return Agent(
            config=self.agents_config['stock_explainer'], # type: ignore[index]
            verbose=True, 
            llm=llm
        )
    ...
    @task
    def news_scan_task(self) -> Task:
        return Task(
            config=self.tasks_config['news_scan_task'], # type: ignore[index]
        )
